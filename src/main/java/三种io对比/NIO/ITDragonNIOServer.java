 ...
 * 从BIO的阻塞到NIO的非阻塞，这是一大进步。功归于Buffer，Channel，Selector三个设计实现。 Buffer ：
 * 缓冲区。NIO的数据操作都是在缓冲区中进行。缓冲区实际上是一个数组。而BIO是将数据直接写入或读取到Stream对象。 Channel ：
 * 通道。NIO可以通过Channel进行数据的读，写和同时读写操作。 Selector ：
 * 多路复用器。NIO编程的基础。多路复用器提供选择已经就绪状态任务的能力。
 * 客户端和服务器通过Channel连接，而这些Channel都要注册在Selector。Selector通过一个线程不停的轮询这些Channel。
 * 找出已经准备就绪的Channel执行IO操作。 NIO通过一个线程轮询，实现千万个客户端的请求，这就是非阻塞NIO的特点。
 */
public class ITDragonNIOServer implements Runnable {
	...
	private void startServer() {
		try {
			...
			/**
			 * SelectionKey.OP_READ : 表示关注读数据就绪事件 SelectionKey.OP_WRITE :
			 * 表示关注写数据就绪事件 SelectionKey.OP_CONNECT: 表示关注socket channel的连接完成事件
	...
	public void run() {
		while (true) {
			try {
				/**
				 * a.select() 阻塞到至少有一个通道在你注册的事件上就绪 b.select(long timeOut)
				 * 阻塞到至少有一个通道在你注册的事件上就绪或者超时timeOut c.selectNow()
				 * 立即返回。如果没有就绪的通道则返回0 select方法的返回值表示就绪通道的个数。
				 */
				// 1.多路复用器监听阻塞
				selector.select();
				// 2.多路复用器已经选择的结果集
