 ...
 * BIO最大的问题是：阻塞，同步。 BIO通讯方式很依赖于网络，若网速不好，阻塞时间会很长。每次请求都由程序执行并返回，这是同步的缺陷。 BIO工作流程：
 * 第一步：server端服务器启动 第二步：server端服务器阻塞监听client请求 第三步：server端服务器接收请求，创建线程实现任务
 */
public class ITDragonBIOServer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/*--------------传统的新增线程处理----------------*/
			/*
			 * while (true) { // 服务器监听：阻塞，等待Client请求 socket = server.accept();
